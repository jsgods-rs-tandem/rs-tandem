...
### 4. Загрузить контент квиза

```bash
npm run seed:content -w @rs-tandem/backend
```

Это заполнит базу категориями, темами и вопросами для квиза. Нужно запускать после каждого `migrate:up`.

### 5. Загрузить контент challenges

```bash
npm run seed:challenges -w @rs-tandem/backend
```

Это заполнит базу категориями и задачами для challenges. Нужно запускать после каждого `migrate:up`.
